/**
 * @file      UART52.h
 * @brief     Interrupt driven serial I/O on UART50.
 * @details   Runs on LM4F120/TM4C123.
 * Uses UART50 to implement bidirectional data transfer to and from a
...
/**
 * @details  Initialize the UART5 using interrupts
 * @note 115,200 baud rate (assuming 80 MHz clock)<br>
 8 bit word length, no parity bits, one stop bit, FIFOs enabled
 * @param  none
 * @return none
 * @brief  Initialize the UART5.
 */
...
/**
 * @details  Wait for new serial port input, interrupt driven
 * @note UART5_InUDec accepts ASCII input in unsigned decimal format<br>
 ...
 * @brief  UART5 input unsigned number.
 */
...
/**
 * @details  Wait for new serial port input, interrupt driven
 * @note UART5_InUHex accepts ASCII input in unsigned hex format<br>
...
void UART5_OutFloat(float val, int n);
