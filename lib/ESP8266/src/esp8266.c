...
void ESP8266_EnableOutput(void) {
	ESP8266_EchoResponse = true;
}

void ESP8266_DisableOutput(void) {
	ESP8266_EchoResponse = false;
}
...
void ESP8266_InitUART(uint32_t baud, int echo){ volatile int delay;
	OS_CurrentThread()->output = OUT_LCD;
  ...
  GPIO_PORTE_DIR_R |= 0x20; // PB5 output to reset
  GPIO_PORTE_PCTL_R =(GPIO_PORTB_PCTL_R&0xFF00FFFF)|0x00110000;
  GPIO_PORTE_DEN_R   |= 0x30; //23
  GPIO_PORTE_DATA_R |= 0x20; // reset high
  UART5_CTL_R &= ~UART_CTL_UARTEN;                  // Clear UART5 enable bit during config
  UART5_IBRD_R = 5000000/baud;
  UART5_FBRD_R = ((64*(5000000%baud))+baud/2)/baud;
   // UART5_IBRD_R = 43;       // IBRD = int(80,000,000 / (16 * 115,200)) = int(43.403)
   // UART5_FBRD_R = 26;       // FBRD = round(0.4028 * 64 ) = 26

  UART5_LCRH_R = (UART_LCRH_WLEN_8|UART_LCRH_FEN);  // 8 bit word length, 1 stop, no parity, FIFOs enabled
  ...
  UART5_IM_R  |= UART_IM_RXIM | UART_IM_RTIM;       // Enable interupt on RX and RX transmission end
...
void UART5_Handler(void){
  if(UART5_RIS_R & UART_RIS_RXRIS){   // rx fifo >= 1/8 full
    UART5_ICR_R = UART_ICR_RXIC;      // acknowledge interrupt
    ESP8266FIFOtoBuffer();
  }
  if(UART5_RIS_R & UART_RIS_RTRIS){   // receiver timed out
    UART5_ICR_R = UART_ICR_RTIC;      // acknowledge receiver time
    ESP8266FIFOtoBuffer();
  }
}
...
void ESP8266FIFOtoBuffer(void){
  ...
  while((UART5_FR_R&UART_FR_RXFE) == 0){
    letter = UART5_DR_R;        // retrieve char from FIFO
