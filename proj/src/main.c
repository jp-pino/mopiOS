...
double Xq=0.1, Yq=0.2, Zq=0.3, xq=0.1, yq=0.2, zq=0.3, aq, bq, cq;
...
void ROSTwistSubscriberExample(void) {
	...
	while(1) {
		...
		if (message) {
			...
			if (twistmessage) {
				// Do whatever
				Xq = twistmessage->linear->x;
				Yq = twistmessage->linear->y;
				Zq = twistmessage->linear->z;
				xq = twistmessage->angular->x;
				xq = twistmessage->angular->y;
				xq = twistmessage->angular->z;
